...
void test_case_client_tcp()
{
    ...
    bzero(&peer_addr, sizeof(peer_addr));
    peer_addr.sin_family = AF_INET;
    peer_addr.sin_addr.s_addr = inet_addr(SER_IP);
    peer_addr.sin_port = htons(SER_TCP_PORT);

    int err = connect(sock, (sockaddr *)&peer_addr, sizeof(peer_addr));
    if (err < 0)
    {
...
void test_case_client()
{
    ...
    snprintf(ser_addr, sizeof(ser_addr), "%s:%d", SER_IP, SER_UDP_PORT);
